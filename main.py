...
BOOTSTRAP_STATE_PATH = Path(__file__).resolve().parent / ".bootstrap_state.json"
BOOTSTRAP_LOG_PATH = Path(__file__).resolve().parent / "install_stemsplat.log"


def _bootstrap_signature() -> dict[str, object]:
    return {
...
def _bootstrap_state_valid() -> bool:
    if not Path("venv").exists() or not BOOTSTRAP_STATE_PATH.exists():
        return False
    try:
        data = json.loads(BOOTSTRAP_STATE_PATH.read_text(encoding="utf-8"))
    except Exception:
        return False
    return isinstance(data, dict) and data == _bootstrap_signature()


def _write_bootstrap_state() -> None:
    BOOTSTRAP_STATE_PATH.write_text(json.dumps(_bootstrap_signature()), encoding="utf-8")
...
OUTPUT_FORMATS = {
    ...
    "m4a_256": ".m4a",
    "flac": ".flac",
    # compatibility aliases for older saved settings values
    "mp3": ".mp3",
    "m4a": ".m4a",
    "ogg": ".ogg",
    "opus": ".opus",
}
DEFAULT_OUTPUT_FORMAT = "mp3_320"
...
DEFAULT_OUTPUT_ROOT = (Path.home() / "Downloads").expanduser()
MODEL_FILE_MAP = {
...
INSTALL_PORT = 6060
MAIN_PORT = 8000
LAUNCH_NOTICE_ENV = "STEMSPLAT_LAUNCH_NOTICE"
launch_notice: dict[str, Any] | None = None
_raw_launch_notice = os.environ.get(LAUNCH_NOTICE_ENV)
if _raw_launch_notice:
    try:
        parsed_notice = json.loads(_raw_launch_notice)
        if isinstance(parsed_notice, dict):
            launch_notice = parsed_notice
    except Exception:
        launch_notice = None
...
def _launch_main_page(port: int | None = None) -> None:
    target_port = int(port or MAIN_PORT)
    main_url = f"http://localhost:{target_port}/"
    for _ in range(40):
        try:
...
def _find_next_available_port(start: int, end: int = 65535) -> int | None:
    for port in range(max(start, 1), end + 1):
        if _port_available(port):
            return port
    return None


def _set_launch_notice(preferred_port: int, selected_port: int) -> None:
    global launch_notice
    if selected_port == preferred_port:
        launch_notice = None
        os.environ.pop(LAUNCH_NOTICE_ENV, None)
        return
    payload = {"requested_port": preferred_port, "selected_port": selected_port}
    launch_notice = payload
    os.environ[LAUNCH_NOTICE_ENV] = json.dumps(payload)


def _run_single_port_install_mode() -> None:
    import uvicorn

    preferred_port = MAIN_PORT
    selected_port = preferred_port
    if not _port_available(preferred_port):
        fallback = _find_next_available_port(preferred_port + 1)
        if fallback is None:
            raise SystemExit("No open port found starting at 8001. Please free a port and try again.")
        selected_port = fallback

    _set_launch_notice(preferred_port, selected_port)
    threading.Thread(target=_launch_main_page, args=(selected_port,), daemon=True).start()
    uvicorn.run("main:app", host="0.0.0.0", port=selected_port, reload=False)
...
class UserSettings:
    ...
    def as_dict(self) -> dict[str, str | bool]:
        return {
            ...
            "preserve_album_art": self.preserve_album_art,
    ...
    def update(
        ...
        *,
        ...
        preserve_album_art: Optional[bool] = None,
        zip_outputs: Optional[bool] = None,
    ) -> None:
        if output_root is not None:
            raw_output = str(output_root).strip()
            candidate = DEFAULT_OUTPUT_ROOT.expanduser() if raw_output in {"", "."} else Path(raw_output).expanduser()
        ...
        if preserve_album_art is not None:
            self.preserve_album_art = bool(preserve_album_art)
        if zip_outputs is not None:
            self.zip_outputs = bool(zip_outputs)
...
def resolve_output_plan(info: dict, *, structure_mode: Optional[str] = None) -> dict[str, Path | str | None]:
    ...
    root = settings.output_root.expanduser()
    _ensure_dir(root)
...
def _is_video_upload(filename: str | None, content_type: str | None = None) -> bool:
...
def _audio_output_extension_for_source(filename: str | None) -> str:
    suffix = Path(filename or "").suffix.lower()
    if suffix in {".m4a", ".mp3", ".flac", ".ogg", ".oga", ".opus"}:
        return suffix
    if suffix == ".aac":
        return ".m4a"
    return ".wav"
...
def _resolve_audio_output_extension(filename: str | None, output_format: str | None) -> str:
    fmt = str(output_format or DEFAULT_OUTPUT_FORMAT)
    if fmt == "mp3":
        fmt = "mp3_320"
    elif fmt == "m4a":
        fmt = "m4a_256"
    mapped = OUTPUT_FORMATS.get(fmt)
    if mapped is None:
        return _audio_output_extension_for_source(filename)
    return mapped
...
def _build_expected_outputs(
    ...
    *,
    ...
    ext = VIDEO_OUTPUT_EXTENSION if as_video else (audio_ext or ".wav")
    expected: list[str] = []
    for stem in stem_list:
        if stem == "deux":
            expected.append(f"{base_stem} - vocals (deux){ext}")
            expected.append(f"{base_stem} - instrumental (deux){ext}")
        else:
            expected.append(f"{base_stem} - {stem}{ext}")
    return expected
...
def _mux_video_with_audio(
    ...
    *,
    ...
    # Fast path: keep video bitstream and only encode replacement audio.
    cmd_copy_video = [
        ffmpeg_path,
        "-hide_banner",
        "-loglevel",
    ...
    if preserve_metadata:
        cmd_copy_video[cmd_copy_video.index("-c:v") : cmd_copy_video.index("-c:v")] = ["-map_metadata", "0"]

    # Fallback: re-encode video for containers/codecs that reject stream copy.
    cmd_reencode_video = [
        ffmpeg_path,
        "-hide_banner",
        "-loglevel",
    ...
    if preserve_metadata:
        cmd_reencode_video[cmd_reencode_video.index("-c:v") : cmd_reencode_video.index("-c:v")] = ["-map_metadata", "0"]

    for cmd in (cmd_copy_video, cmd_reencode_video):
        try:
            subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True)
            ...
            raise AppError(
                ErrorCode.FFMPEG_MISSING, "ffmpeg not found; install ffmpeg and ensure it is on PATH."
            ) from exc
        except Exception as exc:
            logger.debug("video mux command failed: %s", exc)
            try:
                if hasattr(exc, "stderr") and exc.stderr:
                    logger.debug("video mux stderr: %s", exc.stderr)
            except Exception:
                pass
            continue

    raise AppError(ErrorCode.UPLOAD_FAILED, f"Failed to render video output for {audio_track.name}")
...
def _render_video_outputs(
    ...
    *,
    ...
    for stem_name in stem_files:
        if stop_check:
            stop_check()
        audio_path = stems_dir / stem_name
        if not audio_path.exists():
            raise AppError(ErrorCode.INVALID_REQUEST, f"Missing stem for video render: {stem_name}")
        video_name = f"{Path(stem_name).stem}{VIDEO_OUTPUT_EXTENSION}"
        video_path = ensure_unique_path(stems_dir / video_name)
        ...
        rendered.append(video_path.name)
        try:
            audio_path.unlink()
        except Exception:
            logger.debug("failed to remove temporary stem %s after mux", audio_path, exc_info=True)
    return rendered
...
def _apply_source_metadata(
    ...
    *,
    ...
    """
    Best-effort metadata copy from source media to generated output.

    This preserves tags where the destination container supports them.
    """
    if not preserve_metadata:
        return
    if not source_media.exists() or not output_media.exists():
        return
    try:
        ffmpeg_path = _ensure_ffmpeg()
    except AppError:
    ...
    tmp_path = output_media.with_name(f"{output_media.stem}.__meta__{output_media.suffix}")
    base_cmd = [
        ffmpeg_path,
        "-hide_banner",
        "-loglevel",
    ...
    cmds.append([*base_cmd, "-c", "copy", str(tmp_path)])
    ...
    for cmd in cmds:
        try:
            ...
            subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True)
            if tmp_path.exists():
                tmp_path.replace(output_media)
                return
        except Exception as exc:
            logger.debug("metadata pass skipped for %s: %s", output_media, exc)

    try:
        if tmp_path.exists():
            tmp_path.unlink()
    except Exception:
        logger.debug("failed to clean metadata temp file %s", tmp_path, exc_info=True)


def _audio_encode_args_for_ext(ext: str, output_format: str | None = None) -> list[str]:
    ext = (ext or "").lower()
    ...
    if ext == ".mp3":
        if fmt == "mp3_128":
            return ["-c:a", "libmp3lame", "-b:a", "128k", "-id3v2_version", "3", "-write_id3v1", "1"]
        if fmt == "mp3_320":
            return ["-c:a", "libmp3lame", "-b:a", "320k", "-id3v2_version", "3", "-write_id3v1", "1"]
        return ["-c:a", "libmp3lame", "-q:a", "2", "-id3v2_version", "3", "-write_id3v1", "1"]
    if ext == ".m4a":
        if fmt == "m4a_256":
...
def _transcode_with_source_metadata(
    source_media: Path,
    wav_input: Path,
    output_path: Path,
    *,
    ...
    encode_args = _audio_encode_args_for_ext(ext, output_format=output_format)
    ...
    if not preserve_metadata:
        ...
        try:
            subprocess.run(cmd_plain, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True)
            if output_path.exists():
                return
        except Exception as exc:
            logger.debug("audio transcode without metadata failed for %s: %s", output_path, exc)
        raise AppError(ErrorCode.UPLOAD_FAILED, f"Failed to finalize audio for {wav_input.name}")
    ...
    cmd_no_cover = [*base_cmd, *encode_args, str(output_path)]

    cmd_variants = [cmd_with_cover, cmd_no_cover] if preserve_album_art else [cmd_no_cover]
    for cmd in cmd_variants:
        try:
            ...
            logger.debug("audio transcode metadata pass skipped for %s: %s", output_path, exc)

    raise AppError(ErrorCode.UPLOAD_FAILED, f"Failed to finalize tagged audio for {wav_input.name}")


def _finalize_audio_outputs(
    source_media: Path,
    stems_dir: Path,
    stem_files: list[str],
    *,
    ...
    ext = (output_ext or ".wav").lower()
    for stem_name in stem_files:
        if stop_check:
            stop_check()
        wav_path = stems_dir / stem_name
        if not wav_path.exists():
            raise AppError(ErrorCode.INVALID_REQUEST, f"Missing stem output: {stem_name}")
        if ext == ".wav":
            _apply_source_metadata(
                source_media,
                wav_path,
                preserve_metadata=preserve_metadata,
                preserve_album_art=preserve_album_art,
            )
            final_names.append(wav_path.name)
            continue
        out_path = ensure_unique_path(wav_path.with_suffix(ext))
        _transcode_with_source_metadata(
            source_media,
            wav_path,
            out_path,
        ...
        try:
            ...
            logger.debug("failed to remove temporary wav stem %s", wav_path, exc_info=True)
...
def _queue_processing(task_id: str, out_dir: Path, stem_list: list[str]) -> None:
    ...
    def run() -> None:
        try:
            ...
            zip_outputs = bool(tasks.get(task_id, {}).get("zip_outputs", True))
            stems_out = _separate_waveform(manager, audio_path, stem_list, cb, staging_dir, stop_check=_raise_if_stopped)
            preserve_metadata = bool(tasks.get(task_id, {}).get("preserve_metadata", True))
            preserve_album_art = bool(tasks.get(task_id, {}).get("preserve_album_art", True))
            if tasks.get(task_id, {}).get("is_video"):
                source_video = Path(
                    tasks.get(task_id, {}).get("video_src")
                    or tasks.get(task_id, {}).get("orig_src")
                    or ""
                )
                if not source_video.exists():
                    raise AppError(ErrorCode.UPLOAD_FAILED, "Source video missing; please re-upload.")
                cb("video.mux.start", 98)
                stems_out = _render_video_outputs(
                    source_video,
                    staging_dir,
                    stems_out,
                    preserve_metadata=preserve_metadata,
                    stop_check=_raise_if_stopped,
                )
                cb("video.mux.done", 99)
            else:
                source_media = Path(tasks.get(task_id, {}).get("orig_src") or "")
                if source_media.exists() and stems_out:
                    cb("metadata.start", 98)
                    audio_ext = str(tasks.get(task_id, {}).get("audio_ext") or ".wav").lower()
                    stems_out = _finalize_audio_outputs(
                        source_media,
                        staging_dir,
                        stems_out,
                        output_ext=audio_ext,
                        output_format=tasks.get(task_id, {}).get("output_format"),
                        preserve_metadata=preserve_metadata,
                        preserve_album_art=preserve_album_art,
                        stop_check=_raise_if_stopped,
                    )
            ...
            if zip_outputs and plan_mode != "structured" and zip_target is not None and len(stems_out) > 1:
                _raise_if_stopped()
    ...
    if remaining_stems:
        ...
        def process_channel(idx: int, label: str, start: int, end: int) -> None:
            ...
            if need_instrumental_model:
                ...
                # Run instrumental model on the original channel signal (not vocal residual)
                # to avoid over-subtraction/silence when vocals were processed first.
                use_wave = chan_wave
...
async def upload_file(
    ...
    is_video = _is_video_upload(file.filename, file.content_type)
    output_format = settings.output_format
    audio_ext = _resolve_audio_output_extension(file.filename, output_format)
    ...
    tasks[task_id] = {
        ...
        "stem_list": stem_list,
        "is_video": is_video,
        "video_src": str(path) if is_video else None,
        "audio_ext": audio_ext,
        "output_format": output_format,
        "preserve_metadata": bool(settings.preserve_metadata),
        "preserve_album_art": bool(settings.preserve_album_art),
        "zip_outputs": bool(settings.zip_outputs),
        "status": "ready",
...
async def start_task(task_id: str):
    ...
    output_format = settings.output_format
    audio_ext = _resolve_audio_output_extension(info.get("orig_name"), output_format)
    base_stem = Path(info.get("orig_name") or orig_src.name).stem
    tasks[task_id]["audio_ext"] = audio_ext
    tasks[task_id]["output_format"] = output_format
    tasks[task_id]["preserve_metadata"] = bool(settings.preserve_metadata)
    tasks[task_id]["preserve_album_art"] = bool(settings.preserve_album_art)
    tasks[task_id]["zip_outputs"] = bool(settings.zip_outputs)
    tasks[task_id]["stems"] = _build_expected_outputs(
        base_stem,
        info.get("stem_list") or [],
        as_video=bool(info.get("is_video")),
        audio_ext=audio_ext,
    )
...
async def rerun(task_id: str):
    ...
    is_video = bool(old.get("is_video"))
    output_format = settings.output_format
    audio_ext = _resolve_audio_output_extension(old.get("orig_name"), output_format)
    ...
    tasks[new_id] = {
        ...
        "stem_list": stem_list,
        "is_video": is_video,
        "video_src": old.get("video_src") or old.get("orig_src"),
        "audio_ext": audio_ext,
        "output_format": output_format,
        "preserve_metadata": bool(settings.preserve_metadata),
        "preserve_album_art": bool(settings.preserve_album_art),
...
@app.get("/launch_notice")
async def get_launch_notice():
    if not launch_notice:
        return {"notice": None}

    requested_port = int(launch_notice.get("requested_port", MAIN_PORT))
    selected_port = int(launch_notice.get("selected_port", MAIN_PORT))
    return {
        "notice": {
        ...
        "commands": {
            "check": f"lsof -nP -iTCP:{requested_port} -sTCP:LISTEN",
            "terminate": f"sudo lsof -ti tcp:{requested_port} | xargs sudo kill -15",
            "force": f"sudo lsof -ti tcp:{requested_port} | xargs sudo kill -9",
        },
    }
...
async def update_settings(request: Request):
    ...
    preserve_metadata = payload.get("preserve_metadata")
    preserve_album_art = payload.get("preserve_album_art")
    zip_outputs = payload.get("zip_outputs")
    try:
        settings.update(
...
@app.post("/reveal_path")
async def reveal_path(request: Request):
    payload = await request.json()
    raw_path = str(payload.get("path") or "").strip()
    if not raw_path:
        raise AppError(ErrorCode.INVALID_REQUEST, "folder path is required").to_http(400)
    target = Path(raw_path).expanduser()
    if not target.exists() or not target.is_dir():
        raise AppError(ErrorCode.INVALID_REQUEST, "folder doesn't exist").to_http(400)
    ...
    try:
        ...
        logger.warning("failed to reveal folder %s: %s", target, exc)
...
def cli_main(argv: Optional[list[str]] = None) -> None:
    ...
    parser.add_argument("--install", action="store_true", help="Launch app with automatic port selection")
